...
# 第12章（Summarizing and Aggregating）
* `高速パス最適化`
    * データに関する特殊なナレッジを使った、プログラムの実行速度向上手法
    * ~~あんまり詳しく理解できてないので、あとで調べる~~
        * `.set_sorted()`によって指定したカラムにソートフラグを事前につけておくことで、後からソート処理する際に、指定したカラムを無視することができ、それが無駄な計算処理をしない点で高速化できる
* ユーザー定義関数（UDF）
    * `map_elements` -> list型やarray型といった配列の各要素に対して、処理を適用できる
    * `map_batches` -> スカラー型（numpy配列とか）に対して、処理を適用できる
    * `map_groups` -> グルーピングした内容に対して、処理を適用できる
* 時間データを扱うときは、`group_by_dynamic`を使うことで、時間情報を行に割り当てる
    * `group_by`による単なるグルーピングとは別で、取得した時間情報の要素を行に当てるため、行数が増える
# 第13章（Joining and Concatenating）
# 第14章（Reshaping）
* `.partition_by(カラム名, as_dict=True)`<-お気に入り
    * グループ別にDataFrameを保持できる
    * このときのkeyは文字列型ではあるが、タプル型の形状をしているので、その時の指定方法に慣れが必要かも
# 第15章（Visualizing Data）
* `hvPlot`パッケージを使った描画
    * polarsでは、`df.plot.scatter()`のようにDataFrameに描画メソッドを適用できる
    * hvPlotだけでは描画機能を持たず、polarsによって呼び出すことで描画できる
    * インタラクティブな操作ができるグラフ描画も可能（ベースは`bokeh`）
* 同一のデータセットが参照できない都合上、模写コーディングができない..
    * この章を一番楽しみにしていただけ少し残念。これからに期待
    * とりあえず目を通すことは必ずやる
* datetime型を使う時に、polarsでは対応していないものもあるので、そのときは`.to_pandas()`によってpandasに変換するのもあり
    * いずれpolarsでも対応して欲しい
* `hvPlot`で呼び出されるベースのAPIは`bokeh`であるが、`hvplot.extension('matplotlib')`のように設定すれば他の図形描画ライブラリを呼び出せる
* 複数描画したいときは、描画したい式をまとめた最後に`.cols()`を使う
    * 同じグラフに重ねたい場合は、matplotlib同様、そのまま連続して式を記述する
* `grate_tables`パッケージを使うと、より高度な描画が可能
    * 具体的には、グルーピング化した要素ごとのグラフを整列してくれる
    * 習得難易度が少し高そうな印象を受けたので、まずはpolarsとhvplotに慣れてからやってみたい
